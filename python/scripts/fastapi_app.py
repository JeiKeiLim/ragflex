...
from scripts.embedding import ContextManager, get_embeddings
...
class FastAPIApp:
    ...
    def __init__(self):
        ...
        self._router.add_api_route("/uploadfile", self.upload_file, methods=["POST", "OPTIONS"])
    ...
    async def upload_file(self, file: UploadFile = File(...)) -> Dict:
        ...
        pdf_processor = PDFProcessor(content)
        self._context_manager = ContextManager(self._openai_client, pdf_processor.pdf_text)
    ...
    async def query(self, query_data: Dict):
        ...
        context_dist_threshold = query_data.get("context", {}).get("distance_threshold", 0.5)

        query_embedding = get_embeddings(self._openai_client, query)
        index = self._index_manager.search(query_embedding, context_k, context_dist_threshold)
        context = self._context_manager.text_at(index)

        context += "\n" + self._openai_query.query(context, "Summarize the context. Your response must match the language of the context. Do not miss any information.")
